<div id="page-content" class="page-content">
	<section>
		...
		<form class="emergency-edit-form" method="post" action="<?=base_url()?>emergency/updatelog">
			...
			<div class="form-row">
				<span class="form-field-title">Management</span>
				<span class="form-field">
			...
			<div class="form-row">
				...
				<span class="form-field">
					...
					if($log["frn"] == "pending"){
					?>
					<input type="button" value="Accept" onclick="<?if($this->userdetails["agentrole"] == 3){?>acceptfrn('fault_reporting',<?=$log["id"]?>,this)<?}else{?>new Messi('Only ATC is allowed to update FRN.', {title: 'Emergency Log Edit', modal: true});<?}?>" />
					<?
					}else{
					?>
					<span class="frn-status"><?=$log["frn"]?></span>
					<?
					}
					?>
				</span>
			...
			<div class="form-row">
				...
				<span class="form-field">
					...
					<input type="button" class="btn" value="Update" onclick="updateFaultReporting(<?=$log["id"]?>);" />
				</span>
			</div>
		</form>
	</div>
