...
class Report_model extends CI_Model {
	...
	public function getFormLogs($param){
		...
		if( ($param["from"] != "") && ($param["to"] != "") ){
			...
			$sql_datetime = "`datetime` between '".$param["from"]." 00:00:00' AND '".$param["to"]." 23:59:59'";		
		}else if( ($param["from"] == "") && ($param["to"] != "") ){
						
			$sql_datetime = "`datetime` < '".$param["to"]." 23:59:59'";				
		}else if( ($param["from"] != "") && ($param["to"] == "") ){
					
			$sql_datetime = "`datetime` > '".$param["from"]." 00:00:00'";				
		}
	...
	public function getFaultReportingLogs($param){
		...
		generalentry.datetime as ge_datetime, generalentry.actions as ge_actions, 
		generalentry.management as ge_management, */
		$this->db->select("fault_reporting.id as fr_id, fault_reporting.datetime as fr_datetime,fault_reporting.initial as fr_initial,
												fault_reporting.onbehalf as fr_onbehalf, fault_reporting.actions as fr_actions,
												fault_reporting.management as fr_management, fault_reporting.position_name as fr_position_name,
		...
		if( ($param["from"] != "") && ($param["to"] != "") ){
			$this->db->where("`fault_reporting.datetime` between '".$param["from"]." 00:00:00' AND '".$param["to"]." 23:59:59'");		
		}else if( ($param["from"] == "") && ($param["to"] != "") ){
			$this->db->where("`fault_reporting.datetime` < '".$param["to"]." 23:59:59'");				
		}else if( ($param["from"] != "") && ($param["to"] == "") ){
			$this->db->where("`fault_reporting.datetime` > '".$param["from"]." 00:00:00'");				
		}
